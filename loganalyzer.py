...
PATTERN = r"\d{4}-\d{2}-\d{2}(?: [,:0-9TZ]+)?[^ ]* ([A-Z]{4,})"
...
parser.add_argument(
    "-e", "--expand-dir", action="store_true", help="automatically expands dirs"
)
...
def _handle_output(
    output: dict, output_file: str | pathlib.Path | None = None, to_json: bool = True
):
...
def parse_files(
    filepath_list: list[str] | list[pathlib.Path],
    ...
    filepath_list = list(
        filter(_is_text, map(pathlib.Path, [path for path in filepath_list]))
    )
    ...
    try:
        total_counter = reduce(
            lambda x, y: x + y, map(_log2dict, filepath_list), total_counter
        )
