...
def _parse_dir(path: pathlib.Path, verbose: bool):
    ...
    print(stringified_path, "is a directory. It contains", len(file_count), "files and", len(dir_count), "directories")
...
def main():
    ...
    if not args.paths:
        args.paths = [path.strip() for path in sys.stdin.read().split('\n') if path.strip()]
    exit_code = 0
